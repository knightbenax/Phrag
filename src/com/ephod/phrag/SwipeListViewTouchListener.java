...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    public SwipeListViewTouchListener(SwipeListView swipeListView, int swipeFrontView, int swipeBackView) {
    	mp = MediaPlayer.create(swipeListView.getContext(), R.raw.solemn);//initialize the sound file
    	try {
			mp.prepare();
     ...
     * @param position  Position of list
     */
    public void generateDismissAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        int moveTo = 0;
        if (opened.get(position)) {
            if (!swap) {
    ...
    /**
     * Create dismiss animation - Mine.. Ashefor Bezaleel 2014
     *
     * @param view      affected view
     * @param swap      If will change state. If is "false" returns to the original position
     * @param swapRight If swap is true, this parameter tells if move is to the right or left
     * @param position  Position of list
     */
    public void myGenerateDismissAnimate(final View view, final boolean swap, final boolean swapRight, final int position, final EnhancedListAdapter mAdapter, final View motherView) {
        int moveTo = 0;
        if (opened.get(position)) {
            if (!swap) {
                moveTo = openedRight.get(position) ? (int) (viewWidth) : (int) (-viewWidth);
            }
        } else {
            if (swap) {
                moveTo = swapRight ? (int) (viewWidth) : (int) (-viewWidth);
            }
        }
        ...
        animate(view)//.scaleY(0)
                ...
                .setListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        //closeOpenedItems();
                        //performDismiss(view, motherView, position, mAdapter);
                        //animateRemoval(motherView, view, mAdapter);
                        resetCell();
                      
                        //swipeListView.removeView(view);
                    }
                });
	...
	private void animateRemoval(final SwipeListView listview, View viewToRemove, final EnhancedListAdapter mAdapter) {
                ...
                for (int i = 0; i < listview.getChildCount(); ++i) {
                    ...
                    if (startTop != null) {
                        if (startTop != top) {
                            ...
                            child.animate().setDuration(10).translationY(0);
                            if (firstAnimation) {
                        ...
                        int delta = startTop - top;
                        child.setTranslationY(delta);
                        child.animate().setDuration(100).translationY(0);
                        if (firstAnimation) {
                            child.animate().withEndAction(new Runnable() {
                                public void run() {
                                    //mBackgroundContainer.hideBackground();
    ...
    private SortedSet<myPendingDismissData> mPendingDismisses = new TreeSet<myPendingDismissData>();
    
    @SuppressWarnings("unused")
	private void performDismiss(final View dismissView, final View listItemView, final int dismissPosition, final EnhancedListAdapter mAdapter) {

        final ViewGroup.LayoutParams lp = listItemView.getLayoutParams();
        final int originalHeight = listItemView.getHeight();

        ValueAnimator animator = ValueAnimator.ofInt(originalHeight, 1).setDuration(100);

        animator.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {

            	mAdapter.remove(dismissPosition);
            	
                ViewGroup.LayoutParams lp;
                    for (myPendingDismissData pendingDismiss : mPendingDismisses) {
                    ...
                    //resetCell();
                    mp.start(); //Ashefor Bezaleel 15th December 2013/ Play the sound
                }
        ...
        mPendingDismisses.add(new myPendingDismissData(dismissPosition, dismissView, listItemView));
        animator.start();
    ...
    private void generateRevealAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        ...
        if (opened.get(position)) {
            if (!swap) {
                //moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
                moveTo = openedRight.get(position) ? (int) (rightOffset) : (int) (-leftOffset);
            }
        } else {
            if (swap) {
                //moveTo = swapRight ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
                moveTo = swapRight ? (int) (rightOffset) : (int) (-leftOffset);
            }
        }
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (swiping && downPosition != ListView.INVALID_POSITION) {
                    if (opened.get(downPosition)) {
                        ...
                        deltaX += openedRight.get(downPosition) ? rightOffset : -leftOffset;
                    }
                    move(deltaX);
                    return true;
                }
    ...
    public void move(float deltaX) {
        ...
        if (opened.get(downPosition)) {
            ...
            posX += openedRight.get(downPosition) ? -rightOffset : leftOffset;
        }
    ...
    private class myPendingDismissData implements Comparable<PendingDismissData> {
        ...
        myPendingDismissData(int position, View view, View childView) {
            this.position = position;
            this.view = view;
